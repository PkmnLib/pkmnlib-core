...
var Statement = (function () {
	...
	Statement.prototype.mark = function mark() {
		...
		if (this.isReexportDeclaration) {
			var _ret = (function () {
				var id = _this3.module.resolvedIds[_this3.node.source.value];
				var otherModule = _this3.module.bundle.moduleById[id];

				_this3.node.specifiers.forEach(function (specifier) {
					var reexport = _this3.module.reexports[specifier.exported.name];

					reexport.isUsed = true;
					reexport.module = otherModule; // TODO still necessary?

					if (!otherModule.isExternal) otherModule.markExport(specifier.local.name, specifier.exported.name, _this3.module);
				});
	...
	Module.prototype.analyse = function analyse() {
		...
		this.statements.forEach(function (statement) {
			...
			statement.scope.varDeclarations.forEach(function (name) {
				if (reassigned[name] && ! ~_this3.reassignments.indexOf(name)) {
					_this3.reassignments.push(name);
				}
			});
	...
	Module.prototype.bindImportSpecifiers = function bindImportSpecifiers() {
		...
		[this.imports, this.reexports].forEach(function (specifiers) {
			keys(specifiers).forEach(function (name) {
				...
				var id = _this4.resolvedIds[specifier.source];
				specifier.module = _this4.bundle.moduleById[id];
			});
		});

		this.exportAlls.forEach(function (delegate) {
			var id = _this4.resolvedIds[delegate.source];
			delegate.module = _this4.bundle.moduleById[id];
		});

		this.dependencies.forEach(function (source) {
			var id = _this4.resolvedIds[source];
			var module = _this4.bundle.moduleById[id];
	...
	Module.prototype.consolidateDependencies = function consolidateDependencies() {
		...
		this.statements.forEach(function (statement) {
			if (statement.isImportDeclaration && !statement.node.specifiers.length) {
				...
				var id = _this5.resolvedIds[statement.node.source.value];
				var _module = _this5.bundle.moduleById[id];
				...
				keys(statement.stronglyDependsOn).forEach(function (name) {
					...
					addDependency(strongDependencies, _this5.exportDelegates[name]) || addDependency(strongDependencies, _this5.imports[name]);
				});
			}
		});
		...
		this.statements.forEach(function (statement) {
			keys(statement.dependsOn).forEach(function (name) {
				...
				addDependency(weakDependencies, _this5.exportDelegates[name]) || addDependency(weakDependencies, _this5.imports[name]);
			});
		});
		...
		this.statements.forEach(function (statement) {
			...
			if (statement.isImportDeclaration) {
				...
				if (!statement.node.specifiers.length) {
					var id = _this6.resolvedIds[statement.node.source.value];
					var otherModule = _this6.bundle.moduleById[id];
		...
		this.statements.forEach(function (statement) {
			...
			if (statement.isExportDeclaration) {
				...
				// TODO default exports need different treatment
						...
						var canonicalName = _this8.defaultName();
...
var Bundle = (function () {
	...
	Bundle.prototype.build = function build() {
		...
		return _Promise.resolve(this.resolveId(this.entry, undefined, this.resolveOptions)).then(function (id) {
			...
			_this.markAllModifierStatements();
	...
	Bundle.prototype.deconflict = function deconflict(es6) {
		...
		var _loop = function () {
			var module = _this2.orderedModules[i];
		...
		this.orderedModules.forEach(function (module) {
			keys(module.imports).forEach(function (localName) {
				...
				var bundleName = _this2.trace(module, localName, es6);
	...
	Bundle.prototype.render = function render() {
		...
		if (format !== 'es6' && exportMode === 'named') {
			keys(this.entryModule.exports).concat(keys(this.entryModule.reexports)).forEach(function (name) {
				var canonicalName = _this5.traceExport(_this5.entryModule, name);
		...
		var namespaceBlock = this.internalNamespaceModules.map(function (module) {
			var exports = keys(module.exports).concat(keys(module.reexports)).map(function (name) {
				var canonicalName = _this5.traceExport(module, name);
